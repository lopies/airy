...
func bootEtcd() error {
	...
	timeout := global.Cfg.ETCD.DialTimeout
	etcdConfig := lib.ETCDConfig{
		Endpoint:    global.Cfg.ETCD.Endpoint,
		DialTimeout: time.Duration(time.Second.Nanoseconds() * int64(timeout)),
		UserName:    global.Cfg.ETCD.UserName,
		PassWord:    global.Cfg.ETCD.PassWord,
		Prefix:      global.Cfg.ETCD.Prefix,
	}
	if global.ETCD, err = lib.NewETCD(etcdConfig); err == nil {
		logrus.Println("程序载入etcd服务成功")
	}
	return err
}
