...
func Register() *gin.Engine {
	...
	router.Use(middleware.RateLimitMiddleware(time.Second, 100, 10))
	// 404 not found
	...
	var (
		...
		authGroup   = router.Group("/", append(publicMiddleware, middleware.LoginAuth())...)
	)
	// 公用组
	routes.InitCommonGroup(commonGroup)
	// 后台组
	routes.InitAppGroup(authGroup)
	// 赋给全局
