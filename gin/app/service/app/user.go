...
// GetIndex 获取列表
func (s *UserService) GetIndex(requestParams types.IndexRequest) (interface{}, error) {
	var userList = make([]types.UserList, 0)
...
// GetList 获取列表
func (s *UserService) GetList(requestParams types.IndexRequest) (interface{}, error) {
	var userList = make([]types.GinUser, 0)
	pagination, err := paginator.NewBuilder().
		WithDB(global.DB).
		WithModel(models.GinUser{}).
