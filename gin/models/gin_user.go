...
// GinUser 用户表
type GinUser struct {
	Id           uint64 `gorm:"primaryKey;autoIncrement;column:id;type:bigint unsigned;NOT NULL;unique" json:"id"`
	UserName     string `gorm:"column:user_name;type:varchar(64);NOT NULL;unique,comment:用户名" json:"user_name"`              // 用户名
	NickName     string `gorm:"column:nick_name;type:varchar(64);NOT NULL;comment:昵称" json:"nick_name"`                      // 昵称
	RealName     string `gorm:"column:real_name;type:varchar(64);NULL;comment:真实姓名" json:"real_name"`                        // 真实姓名
	Phone        string `gorm:"column:phone;type:varchar(16);NOT NULL;unique,comment:手机号" json:"phone"`                      // 手机号
	Avatar       string `gorm:"column:avatar;type:varchar(128);NULL;comment:头像" json:"avatar"`                               // 头像
